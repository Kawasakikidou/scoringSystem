...
**二期已同步**：四维评分（责任心/时间管理能力/学生工作能力/部门契合度，各 0~25）、
名单管理（新增/改名/改学号/删除，级联提示）、Excel（.xlsx/.xls）导入、附加分单笔 ≤10、
排名同分逐维比较、彻底重置（红色危险入口 + 手输「彻底重置」双重确认）；
一期旧库首次打开会自动迁移并提示（旧单值评分不换算进四维）。改造对照见 `docs/GUI变更步骤.md`。
